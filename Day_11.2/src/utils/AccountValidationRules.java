...
public class AccountValidationRules {
	...
	static {
		...
		MAX_LENGTH = 10;
			...
			beginDate = of(2020,04,01);
			endDate = of(2021,03,31);
...
//add a static method to validate balance : Min balance for any type of a/c =1000
	...
	public static BankAccount validateAccountNo(int acNo,Map<Integer,BankAccount>map) throws AccountHandlingException
	{
//		int index=list.indexOf(new BankAccount(acNo));
//		if(index==-1)
//			throw new AccountHandlingException("A/C no invalid !!!!!!!");
//		return list.get(index);
		BankAccount a=map.get(acNo);//int-->Integer(autoBoxing)-->Object(upcasting).
		if(a==null)
			throw new AccountHandlingException("A/C no invalid !!!!!!!");
		return a;
